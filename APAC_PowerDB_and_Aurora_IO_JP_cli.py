    ...
    for id, df_plant_blk in enumerate(df_list):
        ...
        if offset_balancing_capacity:
            if id == 0:
                ...
                Balancing_MW = get_sql_plant_balancing(src_sql='vAPAC_PowerCapacity_BalancingQty_LIVE', country=country)
...
def get_excel_hydro12mthlyshape(xls_name):
    ...
    df_vec = df_vec.loc[:, ~df_vec.columns.astype(str).str.startswith('Unnamed')]
    df_vec = df_vec.dropna(axis=1, how='all')
...
def get_sql_fuel_to_aid(src_sql, country):
    ...
    sql_qry = """SELECT * FROM """ + src_sql
    df = pd.read_sql_query(sql_qry, engine_src)
    japan_zones = list(dict_zone_mkt.keys())
    mask = (
        (df['Level'] == 'All') |
        ((df['Level'] == 'Country') & (df['LevelName'] == country)) |
        ((df['Level'] == 'ModelZone') & (df['LevelName'].isin(japan_zones)))
    )
    df = df[mask].copy()
...
def debug_read_csv(table_name):
    if not debug_mode:
        return None
    ts = debug_run_ts if debug_run_ts else None
    if not ts:
        return None
    folder = f"debug_{ts}"
    ...
    pattern = os.path.join(folder, f"{safe}*.csv")
    ...
    path = files[-1]
    df = pd.read_csv(path)
    log(f"debug: read CSV {path} rows={len(df)} cols={len(df.columns)} for {table_name}")
    return df

def read_table_for_debug(dest_tbl, engine, sql_qry=None):
    df = debug_read_csv(dest_tbl)
    if df is not None:
        return df
    if engine is None:
        raise RuntimeError(f"engine is None for {dest_tbl}")
    if sql_qry is None:
        sql_qry = f"SELECT * FROM {dest_tbl}"
    return pd.read_sql_query(sql_qry, engine)
...
def validate_excel_transmission(xls_path, sheet_line, sheet_price):
    ...
    df_line = _rename_by_normalized(
        df_line,
        ['Model Start Year', 'Special label for profiling', 'Adjusted Cap', 'Source province', 'Destination province'],
        f"{xls_path}:{sheet_line}",
    )
    _coerce_int(df_line, ['Model Start Year'], f"{xls_path}:{sheet_line}")
    _coerce_numeric(df_line, ['Adjusted Cap'], f"{xls_path}:{sheet_line}")
    ...
    if df_price.empty:
        raise ValueError(f"{xls_path}:{sheet_price} has no data in BU:DX")
...
def run_plants_existing():
    ...
    if should_skip_read(step, [Plant_Existing]):
        log_step(step, "read hash unchanged; skip compute/write")
        return
    TS_Annual_PlantExisting = pd.DataFrame()
    TS_StorageMax_Exist = pd.DataFrame()
...
def run_plants_newbuild():
    ...
    if should_skip_read(step, read_inputs):
        log_step(step, "read hash unchanged; skip compute/write")
        return
    TS_Annual_PlantNew = pd.DataFrame()
    TS_StorageMax_New = pd.DataFrame()
    ...
    Plant_New = Plant_New.drop(['CapacityFactor', 'PlantType-Tech'], axis=1, errors='ignore')
    ...
    if Plant_Wind is not None:
        ...
        PlantWind = PlantWind.drop(['CapacityFactor', 'PlantLife', 'StartYear'], axis=1)
        ...
        log_df_info(f"{step} wind final", PlantWind)
    else:
        PlantWind = None
        tmp_wind_EmissionRate = None
        tmp_wind_EmissionPrice = None
...
def run_resources_load():
    ...
    step = "resources"
    if PlantExisting is None or Plant_New is None:
        log_step(step, "missing inputs; skip compute/write")
        return
    if should_skip_read(step, [PlantExisting, Plant_New, PlantWind, ts_annual_assumptions], ts_annual_module_key="resources"):
        log_step(step, "read hash unchanged; skip compute/write")
        return
    ...
    update_aid_id(dest_tbl='tbl_AID_Time_Series_Annual', dest_df=ts_annual_assumptions, module_key="resources")
...
def run_fuel():
    ...
    Fuel = pd.concat([Fuel, Fuel_Special], axis=0, ignore_index=True)
    log_df_info(f"{step} fuel w/ special", Fuel)
...
def run_constraints():
    ...
    if Fuel is None:
        raise RuntimeError("Fuel is None; run fuel step before constraints")
    Fuel_local = Fuel[['Fuel ID', 'Fuel Name', 'Fuel Type', 'Units', 'Price', 'zREM Topology']].copy()
...
def run_all_modules():
    ...
    if df_resourceGroup is None or df_resource is None or storage_table is None:
        log_step("final_tables", "missing inputs; skip compute/write")
...
def print_banner():
    ...
    lines = [
        border,
        line("WM Aurora data IO for Japan"),
        line("v4.00"),
        border,
    ]
    print("\n".join(lines))
...
def main():
    parser = argparse.ArgumentParser(
        description="Japan-only Aurora IO CLI",
