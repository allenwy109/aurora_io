...
# ---- defaults (match cn_cli) ----
COUNTRY = "China"
AURORA_SQLDB = "ANVDEVSQLVPM01"
AURORA_DBNAME = "Aurora_APAC_DEV_China_test"
SRC_DBNAME = "WM_POWER_RENEWABLES"
INPUT_DIR = r"L:\Power_Renewables\Inputs"
TRANSMISSION_PATH = INPUT_DIR + r"\APAC_Transmission_China.xlsm"
...
def normalize_and_check(df, required, label):
    norm_map = {norm_col(c): c for c in df.columns}
    missing = []
    for req in required:
        if norm_col(req) not in norm_map:
            missing.append(req)
    if missing:
...
def main():
    parser = argparse.ArgumentParser(description="China IO precheck (DB + Excel)")
    ...
    parser.add_argument("--transmission", default=TRANSMISSION_PATH)
    ...
    check_excel(
        args.transmission,
        SHEET_LINE,
        f"Transmission {SHEET_LINE}",
        header=1,
        required=[
            "Model Start Year",
            "Special label for profiling",
            "Adjusted Cap",
            "Source province",
