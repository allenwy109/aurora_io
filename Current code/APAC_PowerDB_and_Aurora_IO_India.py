...
def aggregate_plant_list(df_plantlist, yr_start, yr_end, step, name_prefix, offset_balancing_capacity=False,):
    ...
    # z = df_existing_cummulative[df_existing_cummulative['Capacity'] < 0]
...
def get_sql_windsolarshapes_to_aid_ts_hour(src_sql, country):
    """
    ...
    df['ShapeType'] = df['ShapeType'].str.replace('Sun_Dist', 'PV_D')
    df['ShapeType'] = df['ShapeType'].str.replace('Sun', 'PV')
    df = df.sort_values(by=['Country', 'ModelZone', 'ShapeType', 'ShapeName', 'Year', 'Month', 'DayOfMonth','DayOfWeek',
                            'HourOfDay']).reset_index(drop=True)
    df['AnnualAverageShape'] = df.groupby(['Country', 'ModelZone', 'ShapeType', 'ShapeName', 'Year'], as_index=False)[
        'AID_HourlyShape'].transform('mean')
    df.rename(columns={'ShapeType': 'PlantType', 'AID_HourlyShape': 2019}, inplace=True)
    ...
    df = pd.melt(df, id_vars=['Country', 'ModelZone', 'PlantType', 'ShapeName', 'Month', 'DayOfMonth','DayOfWeek', 'HourOfDay',  'HourOfYear','AnnualAverageShape'],
                 var_name='Year', value_name='AID_HourlyShape')
    ...
    df['Final_CF'] = np.nan
    df[['Global_CF', 'Country_CF', 'Zone_CF', 'Final_CF']] = df[['Global_CF', 'Country_CF', 'Zone_CF', 'Final_CF']].fillna(method='ffill', axis=1)
    df['FinalAID_Shape'] = (df['AID_HourlyShape'] / df['AnnualAverageShape']) * df['Final_CF']
    # debug CF annual changes
    df = df.sort_values(by=['Country', 'PlantType', 'ShapeName', 'Year', 'Month','DayOfMonth', 'DayOfWeek','DayOfMonth',
                            'HourOfDay']).reset_index(drop=True)
    df_debug = df[df['PlantType'] == 'Wind_Onshore'].reset_index(drop=True)
    df_debug = df_debug[df_debug['ModelZone'] == 'TAS'].reset_index(drop=True)

    # get hourly time series df
    df = df.drop(['Country', 'Global_CF', 'Country_CF', 'Zone_CF', 'Final_CF', 'AnnualAverageShape', 'AID_HourlyShape'], 1)
    df.rename(columns={'ModelZone': 'Area', 'FinalAID_Shape': 'AID_HourlyShape'}, inplace=True)
    df = df.sort_values(by=['Area', 'PlantType', 'ShapeName', 'Year', 'Month', 'DayOfWeek','DayOfMonth', 'HourOfYear',
                            'HourOfDay']).reset_index(drop=True)

    # NY:no need to merge the 168,the sql table includes the [HourOfYear] column, so no need the merge to get this column
    # df = pd.merge(df, df_168map, how='left', on=['DayOfWeek', 'HourOfDay'])
    # df = df.drop(['DayOfWeek', 'HourOfDay'], 1)


    df['AID_HourlyShape'] = 100 * (1 - df['AID_HourlyShape'])
    # df = pd.pivot_table(df, index=['Year', 'Area', 'PlantType', 'ShapeName', 'Month'],
    #                             columns='HrOfWk',
    #                             values='AID_HourlyShape',
    #                             fill_value=0).reset_index()
    ...
    # Hourly to monthly time series
    df_monthly = df[['Year', 'Area', 'PlantType', 'ShapeName_mth', 'Month', 'Name']].reset_index(drop=True)
    df_monthly = df_monthly.drop_duplicates()

    df_monthly.rename(columns={'ShapeName_mth': 'ShapeName'}, inplace=True)
    df_monthly['Name'] = 'hr_' + df_monthly['Name']
    df_monthly = df_monthly.set_index(['Year', 'Area', 'PlantType', 'ShapeName'])
    df_monthly = df_monthly.pivot(columns='Month').reset_index()
    ...
    # get the hourly shape table as expected
    df['ID'] = df['Name']
    df = df.drop(['DayOfWeek','PlantType','ShapeName_mth'], 1)
    df = pd.pivot_table(df, index=['ID','Area',  'Name','ShapeName', 'Month', 'DayOfMonth', 'HourOfDay'],
                        columns='Year',
                        values='AID_HourlyShape',
                        fill_value=0).reset_index()

    df_maint_hr = df[['Area', 'ShapeName', 'ID']].reset_index(drop=True)
    df_maint_hr['ID'] = 'hr_' + df_maint_hr['ID']
    df_maint_hr= df_maint_hr.drop_duplicates()




    return df, df_monthly, df_maint_mth, df_maint_hr
...
if country == 'India':
    TS_Hourly, TS_Monthly, Maint_mth, Maint_hr = get_sql_windsolarshapes_to_aid_ts_hour(
        src_sql='vAPAC_Shapes_8760_LatestYear_ModelZone',
        country=country)
else:
...
if country =='India':
    TS_Hourly['ID'] = TS_Hourly['ID'].str.replace('East Central South Kalimantan', 'ECS Kalimantan')
    TS_Hourly['ID'] = TS_Hourly['ID'].str.replace('Papua Timor Maluku Nusa Tenggara', 'Timor')
else:
...
if country == 'China':
    Plant_Existing, TS_Annual_PlantExisting, TS_StorageMax_Exist = aggregate_plant_list(df_plantlist=Plant_Existing, name_prefix='Existing',
                                                                                        yr_start=2020, yr_end=yr_end, step=5,
...
if country == 'China':
    Plant_New, TS_Annual_PlantNew, TS_StorageMax_New = aggregate_plant_list(df_plantlist=Plant_New, name_prefix='NewBuild', yr_start=2000,
                                                                            yr_end=yr_end,  step=5, offset_balancing_capacity=False)
...
if country =='India':
    # TS_Hourly = TS_Hourly.drop(['Month'], 1)
    TS_Hourly.rename(columns={'Area': 'zREM Topology',
                              'ShapeName': 'zREM Type',
                              'DayOfMonth': 'Day',
                              'HourOfDay': 'Hour',}, inplace=True)
    TS_Monthly.rename(columns={'Area': 'zREM Topology', 'ShapeName': 'zREM Type'}, inplace=True)

    reload_tbl(engine=engine_dest, dest_tbl='tbl_AID_Time_Series_Hourly', dest_df=TS_Hourly)
