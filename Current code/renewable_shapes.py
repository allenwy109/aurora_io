...
def get_renewable_shape(src_type, sheet_name, shapetype, shapename, levelname, level):
    """
    ...
    if src_type == 'excel':
        ...
        df = pd.DataFrame(sht.range('A4').expand().value)
        df.columns = df.iloc[0, :]
        df = df.iloc[1:, 1:].reset_index(drop=True)
        df['LevelName'] = levelname
        df['Level'] = level
        df['ShapeType'] = shapetype
        df['ShapeName'] = shapename
        df['local_time'] = pd.to_datetime(df['local_time'])
        df['electricity'] = pd.to_numeric(df['electricity'])
        df.rename(columns={'local_time': 'Date', 'electricity': 'Shape'}, inplace=True)
...
if __name__ == '__main__':
    ...
    df_solar = get_renewable_shape(src_type='excel', sheet_name='SUN_South', shapetype='Sun', shapename='NoTracking_Tilt22', levelname='South New '
                                                                                                                                       'Zealand',
                                   level='ModelZone')
    upload_to_db(engine=engine_sqldb, dest_df=df_solar, dest_tbl='APAC_Shapes_Raw8760_LIVE', existMethod='append')


    df_onshore = get_renewable_shape(src_type='excel', sheet_name='Onshore_Wind_South', shapetype='Wind_Onshore', shapename='H90m_Vestas_V90 2000',
                                     levelname='South New Zealand', level='ModelZone')
    upload_to_db(engine=engine_sqldb, dest_df=df_onshore, dest_tbl='APAC_Shapes_Raw8760_LIVE', existMethod='append')


    df_offshore = get_renewable_shape(src_type='excel', sheet_name='Offshore_Wind_South', shapetype='Wind_Offshore', shapename='H120m_Vestas_V164 '
                                                                                                                              '9500',
                                      levelname='South New Zealand',  level='ModelZone')
